...
func DeployHelmChartTask(tf *flow.TaskFlow, release *helm.Release) *flow.Task {
	return tf.NewSubflow(fmt.Sprintf("deploy-%s", release.Name), func(sf *flow.Subflow) {
		cond := sf.NewCondition(fmt.Sprintf("check-release-exists-%s", release.Name), func() uint {
		...
		cond.Precede(
			sf.NewTask(fmt.Sprintf("install-release-%s", release.Name), func() {
			...
			sf.NewTask(fmt.Sprintf("upgrade-release-%s", release.Name), func() {
